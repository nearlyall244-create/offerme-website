...
import { Plus, Trash2, RefreshCw, Tag, Calendar, MapPin, Phone, CheckCircle } from 'lucide-react'
import SuccessModal from '@/components/shared/SuccessModal'
import styles from './AdminViewPosts.module.css'

const MAX_FILE_SIZE = 5 * 1024 * 1024
const ALLOWED_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp']

export default function AdminViewPosts() {
  ...
  const getStatusBadge = (post) => {
    ...
    if (status === 'pending') return <span className={`${styles.statusBadge} ${styles.statusPending}`}>Pending</span>
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>View and manage your admin-created posts</p>
        </div>
        <Link to="/admin/dashboard/my-post" className={styles.addBtn}>
          <Plus size={18} />
          New Post
        </Link>
      </div>
                  ...
                  <>
                    ...
                    <div className={styles.cardBody}>
                      <div className={styles.cardHeader}>
                        <div>
                          <span className={styles.shopName}>{post.businesses?.shop_name || 'Admin Post'}</span>
