export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (mine === 'true') {
        ...
        const { data: businesses } = await supabaseAdmin
          ...
          .select('id, shop_name, shop_address, enquiry_number, shop_image_url, category_id, subcategory_id, business_email, shop_description, status, is_active')
    ...
    // ── PUT → update sell-business post (resets to pending for admin approval) ──
    if (req.method === 'PUT') {
      const { verifyToken } = await import('../_lib/verifyToken.js')
      const authResult = await verifyToken(req)
      if (authResult.error) return res.status(authResult.status).json({ error: authResult.error })
      ...
      if (!owner) return res.status(400).json({ error: 'Business owner not found' })
      ...
      if (offer.business_id) {
        ...
        bizFields.status = 'pending'
        bizFields.is_active = false
        ...
        if (Object.keys(bizFields).length > 0) {
          const { error: updateBizErr } = await supabaseAdmin
            ...
            .eq('id', offer.business_id)
          if (updateBizErr) {
            console.error('[offers] PUT sell-business update error:', updateBizErr)
            return res.status(500).json({ error: `Failed to update business: ${updateBizErr.message}` })
          }
        }
      }

      // Update offers_post table
      const offerFields = {}
      ...
      if (description !== undefined) {
        ...
        offerFields.description = fullDesc || null
      }
      ...
      if (Object.keys(offerFields).length > 0) {
        const { error: updateOfferErr } = await supabaseAdmin
          ...
          .eq('id', offerId)
        if (updateOfferErr) {
          console.error('[offers] PUT offer update error:', updateOfferErr)
      ...
      return res.status(200).json({
        success: true,
        message: 'Post updated and submitted for admin approval',
      })
    }
